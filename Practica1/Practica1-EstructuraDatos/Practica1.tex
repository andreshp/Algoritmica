...
ordenación de la burbuja. Para ello considere que \(f(x)\) es de la
forma \(ax^2+bx+c\).
...
\(T: \mathbb{N} \rightarrow \mathbb{N}\) la función tal que dado el
número de elementos del vector nos proporciona la eficiencia del
algoritmo en el caso de que no se encuentre un elemento dado. Es claro
que \(T(1) = O(1)\) pues solo se comprueba dicho elemento. Para
\(n > 1\) podemos expresar \(T\) de forma recursiva:
\[ T(n) = 1 + T(n/2) \] La explicación es sencilla, para un vector de
tamaño n se realiza una comprobación y se aplica el algoritmo en un
subvector de tamaño la mitad. Por tanto, basta resolver la anterior
ecuación recurrente para obtener la eficiencia teórica. Tomando
\(m = log_2 n\) se tiene que
...
\(m \in \mathbb{N}\) se tiene \(x_{m+1} = 1 + x_m = 1 + m\) como se
quería. Volviendo a la función \(T\):
...
Se puede observar un valor casi nulo con una variación de \(10^{-6}\).
La razón es bien sencilla: la precisión del clock de c++ es muy
limitada. Puesto que la búsqueda binaria es muy rápida por ser
logarítmica, el tiempo tomado tiende a 0 y clock no es capaz de medirlo.
Para resolver este problema he creado un nuevo código
\textbf{busqueda\_binaria.cpp} que contiene el algoritmo dado y lo
ejecuta 10000 veces, calculando el tiempo como la media de las 10000
reiteraciones. De esta forma sí se consigue medir el tiempo con clock
para las 10000 ejecuciones. Se muestran los resultados en la siguiente
imagen:
...
ordenación de eficiencia O(\(n^2\)) y que suele ser de los mejores con
esa eficiencia. El cálculo teórico de la misma es sencillo.
...
torno a \(101 2^n\) se producirá este fenómeno, mejorando radicalmente
...
lo hace Heapsort pues a pesar de ser O(\(nlog_2 n\)), en primer lugar
crea el heap y en segundo lugar lo ordena. Por ello obtiene tiempos dos
veces más lento.
