...
En este apartado vamos a buscar soluciones teóricas al problema del viajante de comercio mediante el empleo de dos algoritmos, una que emplee la técnica $Branch and Bound$ y otro $Backtracking$.

Para ambos debemos de tener en cuenta que debemos usar una función de acotación, que permita determinar qué ramas pueden ser correctas para una solución óptima y cuales no merecen la pena resolver. Así, para obtener esta acotación empleando la técnica $Branch and Bound$ debemos definir una función estimar, que permita obtener la estimación de la mejor solución con el empleo de unos u otros nodos.
...
def estimar()
  ...
  estimacion += distancias.minimo  \
            (solucion_actual.primeraVisitada,noVisitadas)
  estimacion += distancias.minimo  \
            (solucion_actual.ultimaVisitada,noVisitadas)
...
def TSPBacktracking(candidatos, sol_actual, mejor_solucion)
  ...
  for i in 0..candidatos.size
    ...
    if sol_actual < mejor_solucion
      resolver(candidatos,sol_actual,mejor_solucion)
    end
...
Hasta que no esté todo correcto no se pueden hacer gráficas. Apartado por rellenar.
